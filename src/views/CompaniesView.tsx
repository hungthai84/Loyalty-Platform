...
export function CompaniesView({ embedded = false }: { embedded?: boolean }) {
 ...
 {company.type === 'branch' ? (
 <div className="mt-1.5 flex items-center gap-1 text-xs uppercase font-bold text-blue-500 bg-blue-500/10 px-2 py-0.5 rounded-full w-fit">
 Chi nhánh • {companies.find(c => c.id === company.parentId)?.name || 'Chưa rõ công ty'}
 </div>
 ) : (
 <div className="mt-1.5 flex items-center gap-1 text-xs uppercase font-bold text-amber-500 bg-amber-500/10 px-2 py-0.5 rounded-full w-fit">
 Công ty Quản lý
 </div>
 )}
 ...
 <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">156 Khách hàng</span>
 </div>
 </div>
 </div>
 </Card>
