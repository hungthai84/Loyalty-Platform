    ...
    <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8 min-h-screen space-y-6">
      ...
      <div className="w-full max-w-[400px] bg-card border border-border/70 rounded-2xl p-5 shadow-lg space-y-4">
        <div className="flex items-center justify-between border-b pb-3 border-border/40">
          ...
          <span className="text-[10px] bg-[#2f6cf5]/10 text-[#2f6cf5] font-black px-2.5 py-0.5 rounded-full uppercase">Hệ Thống (Mặc định)</span>
        </div>
              ...
              <>
                ...
                <div className="mt-4 text-left">
                  <h2 className={`text-3xl font-heading ${textPrimary} leading-tight`}>
                    Chào bạn, <br/>
                    <span className="text-[#2f6cf5] font-serif italic text-4xl">Eleanor.</span>
                  </h2>
                ...
                <div className={`mt-6 aspect-[1.586/1] ${cardGradient} rounded-2xl relative overflow-hidden shadow-xl flex flex-col justify-between p-6 transition-all duration-300`}>
                  ...
                  <div className="relative z-10 text-left">
                    <p className="text-[10px] tracking-[0.2em] text-white/50 uppercase font-bold">Số điểm hiện có</p>
                    <p className="text-white text-4xl font-mono mt-1 font-black">{customerPoints.toLocaleString()}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  >
                    ...
                    <span className={`text-[10px] ${textPrimary} font-bold tracking-wide uppercase`}>Đổi quà</span>
                  </button>
                  <button 
                    className={`flex flex-col items-center justify-center p-4 rounded-2xl ${buttonBg} cursor-pointer`}
                  ...
                  >
                    ...
                    <span className={`text-[10px] ${textPrimary} font-bold tracking-wide uppercase`}>Lịch sử</span>
                  </button>
                  <button 
                    className={`flex flex-col items-center justify-center p-4 rounded-2xl ${buttonBg} cursor-pointer`}
                  >
                    <UserIcon className="w-6 h-6 text-[#2f6cf5] mb-2" />
                    <span className={`text-[10px] ${textPrimary} font-bold tracking-wide uppercase`}>Hồ sơ</span>
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button className="text-[10px] text-[#2f6cf5] font-black uppercase hover:underline">Khám phá</button>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className={isPortalDark ? "bg-gradient-to-br from-indigo-500/20 to-purple-500/10 p-4 rounded-2xl border border-indigo-500/20 text-left" : "bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100/80 text-left"}>
                      ...
                      <p className={`${isPortalDark ? 'text-indigo-300' : 'text-indigo-600'} text-[9px] mt-1 uppercase font-extrabold tracking-tighter`}>+500 PTS / LẦN</p>
                    </div>
                    <div className={isPortalDark ? "bg-gradient-to-br from-emerald-500/20 to-teal-500/10 p-4 rounded-2xl border border-emerald-500/20 text-left" : "bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100/80 text-left"}>
                      ...
                      <p className={`${isPortalDark ? 'text-emerald-300' : 'text-emerald-600'} text-[9px] mt-1 uppercase font-extrabold tracking-tighter`}>ƯU TIÊN DIAMOND</p>
                    </div>
                  </div>
                </div>
                ...
                <div className={`${cardBg} rounded-3xl p-6 transition-colors duration-300`}>
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <button className="text-[10px] text-[#2f6cf5] font-black uppercase tracking-wider hover:underline">Xem tất cả</button>
                  </div>
                      ...
                      <React.Fragment key={i}>
                        <div className="flex justify-between items-center">
                          <div className="flex flex-col text-left">
                            ...
                            <span className={`text-[10px] ${textMuted} mt-1 uppercase tracking-tight`}>{act.date}</span>
                          </div>
                          <span className={act.type === 'earn' ? 'text-[#2f6cf5] text-sm font-mono font-bold' : `${isPortalDark ? 'text-zinc-500' : 'text-zinc-400'} text-sm font-mono`}>
                            {act.points} pts
                          </span>
                        </div>
                  ...
                  <div className="space-y-3 pt-1">
                    <div className="flex items-center justify-between text-left">
                      <div>
                        ...
                        <span className={`text-[10px] ${textMuted}`}>Cáp màu màn hình yêu thích</span>
                      </div>
              ...
              <div className="space-y-4 pt-2">
                <div className="flex items-center justify-between px-1">
                  ...
                  <div className="text-right">
                    <p className={`text-[10px] ${textSecondary} uppercase font-bold`}>Số dư</p>
                    <p className="text-[#2f6cf5] font-mono font-bold text-base">{customerPoints.toLocaleString()} pts</p>
                  </div>
                </div>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0 text-left">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-[#2f6cf5] text-xs font-mono font-black">{rule.pointsRequired.toLocaleString()} pts</span>
                              <div className="w-1 h-1 rounded-full bg-zinc-700"></div>
                              <span className={`${textSecondary} text-[10px] font-bold`}>
                          ...
                          <button 
                            ...
                            className={`px-3.5 py-1.5 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer shrink-0 ${
                              canRedeem 
                    ...
                    <div key={item.id} className={`${cardBg} rounded-2xl p-4 space-y-3 transition-colors duration-300 text-left`}>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className={`${textMuted} text-[10px]`}>{item.date} • {item.value}</p>
                        </div>
                      </div>
                      <div className={`p-2.5 rounded-xl ${isPortalDark ? 'bg-zinc-900/50' : 'bg-zinc-50'} border border-dashed border-border flex items-center justify-between gap-1`}>
                        <span className="text-[10px] text-muted-foreground font-semibold">MÃ QUÀ KHẢ DỤNG</span>
                        <span className={`font-mono font-bold text-xs ${isPortalDark ? 'text-amber-400' : 'text-amber-600'} tracking-wider`}>{item.code}</span>
                      </div>
                    </div>
                  ))}
                </div>
