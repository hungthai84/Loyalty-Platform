...
import { doc, setDoc, serverTimestamp, deleteDoc, collection, query, getDocs, onSnapshot } from 'firebase/firestore';
...
const PRESET_COLORS = [
  '#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899', '#6366f1', '#14b8a6', '#f43f5e'
];
...
export function TierConfigDialog({ onClose, tier }: TierConfigDialogProps) {
  ...
  const [conditions, setConditions] = useState<TierCondition[]>(tier?.conditions || []);
  ...
  const addCondition = () => {
    setConditions([...conditions, { field: 'points', operator: 'gte', value: 0 }]);
  };
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 p-4 backdrop-blur-md">
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-border bg-muted/20">
          <div>
            <h3 className="text-xl font-bold font-heading">{tier ? 'Cấu hình Hạng VIP' : 'Tạo Hạng VIP mới'}</h3>
            <p className="text-xs text-muted-foreground mt-0.5">Xác định đặc quyền và điều kiện thăng hạng động.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-8 pt-6 space-y-8 custom-scrollbar">
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Tên hạng</label>
              <input 
                required
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Hệ số tích lũy (x)</label>
              <input 
                type="number"
                step="0.1"
                required
                className="w-full px-4 py-3 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary/20 outline-none font-mono"
                value={multiplier}
                ...
                {PRESET_COLORS.map(c => (
                  <button
                    ...
                    className={cn(
                      ...
                      color === c ? "border-foreground scale-110 shadow-lg" : "border-transparent scale-100"
                    )}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div className="p-4 bg-muted/30 rounded-2xl border border-border space-y-2">
                 <p className="text-xs font-medium">Ngưỡng điểm mặc định (để hiển thị dashboard)</p>
                 <input 
                    type="number"
                    className="w-full px-4 py-2 bg-background border border-border rounded-lg outline-none font-mono"
                    value={threshold}
              ...
              {conditions.map((condition, idx) => (
                <div key={idx} className="flex gap-2 p-4 bg-background border border-border rounded-2xl relative group">
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div className="relative">
                       <input 
                          ...
                          type={condition.field === 'custom_attribute' ? 'text' : 'number'}
                    ...
                    {condition.field === 'custom_attribute' && (
                       <div className="md:col-span-3 mt-1">
                          <select 
                            className="w-full bg-muted/50 px-3 py-2 rounded-lg text-[10px] font-bold uppercase border-none outline-none"
                            value={condition.attributeKey || ''}
                            onChange={e => updateCondition(idx, { attributeKey: e.target.value })}
                          >
                             <option value="">Chọn thuộc tính tùy chỉnh...</option>
                             {attributes.map(attr => (
                               <option key={attr.id} value={attr.key}>{attr.label} ({attr.key})</option>
                             ))}
                          </select>
                       </div>
                    )}
