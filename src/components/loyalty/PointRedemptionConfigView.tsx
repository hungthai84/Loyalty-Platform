...
const REWARD_TYPE_OPTIONS = [
  { value: 'discount', label: 'Phiếu giảm giá (VND)', icon: Ticket, color: 'text-blue-500 bg-blue-550/10' },
  { value: 'voucher', label: 'Voucher Chiết khấu (%)', icon: Percent, color: 'text-amber-500 bg-amber-550/10' },
  { value: 'item', label: 'Quà hiện vật & Sản phẩm', icon: Package, color: 'text-emerald-500 bg-emerald-550/10' },
  { value: 'service', label: 'Dịch vụ & Đặc quyền đặc biệt', icon: Sparkles, color: 'text-purple-500 bg-purple-550/10' }
];
...
const PRESET_RULES = [
  {
    ...
    rewardValue: 50000,
    description: "Áp dụng cho mọi hóa đơn mua hàng từ 250k trở lên. Hạn dùng 30 ngày.",
    isEnabled: true,
    minBillValue: 250000,
    expiryDays: 30
  },
  ...
  },
  {
    ...
    rewardValue: 1,
    description: "Quà tặng hiện vật đặc quyền in logo thương hiệu SEVA cao cấp.",
    isEnabled: true,
    minBillValue: 0,
    expiryDays: 60
  },
  {
    ...
    rewardValue: 1,
    description: "Tặng 1 buổi trải nghiệm dịch vụ chăm sóc da chuyên sâu 60 phút miễn phí.",
    isEnabled: true,
    minBillValue: 0,
    expiryDays: 90
  }
...
export function PointRedemptionConfigView() {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'create_offers' | 'redeem_terminal'>('create_offers');
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      console.error("Error listening to redemption rules:", error);
      setLoadingRules(false);
    });
  ...
  const handleBootstrap = async () => {
    if (!user) return;
    if (!confirm("Bạn có muốn tạo thêm các quy tắc đổi quả mẫu chuẩn không?")) return;
  ...
  const handleToggleRuleStatus = async (rule: any) => {
    ...
    try {
      ...
      toast.success(`Đã ${nextState ? "kích hoạt" : "tạm ngưng"} quy tắc đổi quà: ${rule.name}`);
    } catch (e) {
      console.error(e);
  ...
  const handleDeleteRule = async (id: string, name: string) => {
    if (!user) return;
    if (!confirm(`Bạn có chắc muốn xóa cấu hình quà tặng "${name}" không?`)) return;
  ...
  const handleAdjustCustomerPoints = async (customer: any, amount: number) => {
    ...
    try {
      ...
      toast.success(`Đã cập nhật điểm cho khách hàng ${customer.name}: ${finalPoints.toLocaleString()} pts`);
    } catch (err) {
      console.error(err);
  ...
  const handleExecuteRealRedeem = async (customer: any, rule: any) => {
    ...
    if (currentPoints < rule.pointsRequired) {
      toast.error(`Khách hàng không đủ điểm tích luỹ. Cần thêm ${rule.pointsRequired - currentPoints} pts.`);
      return;
    }

    if (!confirm(`Bạn có chắc chắn muốn tiến hành đổi ưu đãi "${rule.name}" cho khách hàng ${customer.name}? Việc này sẽ khấu trừ ${rule.pointsRequired} pts khỏi tổng tích lũy.`)) {
      return;
    }
    ...
    try {
      ...
      toast.success(
        <div className="space-y-1 text-left select-none">
          <p className="font-bold text-emerald-600">🎉 Đã Đổi Ưu Đãi Thành Công!</p>
          <p className="text-xs">Đã đổi: <strong>{rule.name}</strong></p>
          <p className="text-xs">Khách hàng: <strong>{customer.name}</strong></p>
          <p className="text-[10px] text-muted-foreground font-mono">
            Đã trừ <span className="text-rose-600 font-bold">-{rule.pointsRequired} pts</span>. Điểm còn lại: {newPoints} pts
          </p>
        </div>,
        { duration: 5000 }
      );
  ...
  const handleSimulateClaim = (rule: any) => {
    ...
    if (pts < rule.pointsRequired) {
      toast.error(`Không đủ điểm! Cần thêm ${rule.pointsRequired - pts} điểm để đổi quầy quà tặng này.`);
      return;
    }
    ...
    toast.success(
      <div className="space-y-1 text-left">
        <p className="font-bold text-emerald-600">🎉 Đổi Quà Thử Nghiệm Thành Công!</p>
        <p className="text-xs">Bạn đã đổi quà thành công: <strong>{rule.name}</strong></p>
        <p className="text-[10px] text-muted-foreground font-mono">Đã khấu trừ: -{rule.pointsRequired} pts. Điểm ảo còn lại: {nextPoints} pts</p>
      </div>,
      { duration: 4000 }
    );
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-primary/5 p-6 border border-primary/10 rounded-3xl">
        <div className="flex items-start gap-4 text-left">
          ...
          <div className="space-y-1">
            <h4 className="font-bold text-lg text-foreground">Hệ thống Thiết lập & Đổi quà Ưu đãi</h4>
            <p className="text-xs text-muted-foreground leading-relaxed max-w-2xl">
              Quy trình khép kín: Tổ chức các chương trình quà贈 ưu đãi trước (Voucher, Hiện vật, Dịch vụ). Sau đó chuyển sang quầy đổi quà để khách đổi điểm & tự động khấu trừ điểm tích lũy của khách hàng trên hệ thống.
            </p>
          </div>
        </div>
          ...
          <div className="xl:col-span-8 space-y-4">
            <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between px-1">
              <div className="text-left">
                <h5 className="text-[11px] font-extrabold text-[#2f6cf5] uppercase tracking-widest">
                  Thư viện quà tặng tích lũy ({filteredRules.length})
                </h5>
                <p className="text-[11px] text-muted-foreground">Tạo các gói kích cầu mua sắm bằng điểm của khách hàng</p>
              </div>
            ...
            {loadingRules ? (
              ...
              <div className="py-16 text-center border border-dashed border-border rounded-3xl space-y-3 bg-card/40">
                ...
                <p className="text-sm text-muted-foreground font-medium">Chưa có quy định ưu đãi quà tặng nào.</p>
              ...
              <div className="space-y-3">
                {filteredRules.map((rule: any) => {
                  const typeInfo = REWARD_TYPE_OPTIONS.find(o => o.value === rule.rewardType) || REWARD_TYPE_OPTIONS[0];
                      ...
                      <CardContent className="p-5 space-y-4">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex items-start gap-4">
                            <div className={`p-2.5 rounded-xl ${typeInfo.color} shrink-0 mt-0.5`}>
                            ...
                            <div className="space-y-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {rule.isEnabled === false && (
                                  <span className="bg-rose-500/10 text-rose-600 font-extrabold text-[8px] tracking-wider px-1.5 py-0.2 rounded uppercase">Vô hiệu hóa</span>
                                )}
                              </div>
                              <p className="text-xs text-muted-foreground leading-relaxed max-w-md">
                                {rule.description || "Không có mô tả chi tiết cho món quà này."}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            <button
                              ...
                              title={rule.isEnabled !== false ? "Tạm ngưng" : "Kích hoạt"}
                            >
                              <Power className={`w-3.5 h-3.5 ${rule.isEnabled !== false ? 'text-emerald-500' : 'text-muted-foreground'}`} />
                            </button>
                            <button
                              type="button"
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 pt-1 border-t border-border/10">
                          <div className="p-2 bg-primary/5 rounded-xl border border-primary/10 flex flex-col justify-between">
                            <span className="text-[9px] text-muted-foreground font-semibold uppercase tracking-wider block">Yêu cầu đổi</span>
                            <span className="text-xs font-extrabold text-primary font-mono mt-0.5 flex items-center gap-1">
                          ...
                          <div className="p-2 bg-muted/30 rounded-xl border border-border/10 flex flex-col justify-between">
                            <span className="text-[9px] text-muted-foreground font-semibold uppercase tracking-wider block">Giá trị gói quà</span>
                            <span className="text-xs font-bold text-foreground font-mono mt-0.5">
                              {rule.rewardType === 'discount' ? `${rule.rewardValue?.toLocaleString()}₫` :
                               rule.rewardType === 'voucher' ? `${rule.rewardValue}%` :
                               'Điện thoại / Vật phẩm'}
                            </span>
                          </div>

                          <div className="p-2 bg-muted/30 rounded-xl border border-border/10 flex flex-col justify-between">
                            <span className="text-[9px] text-muted-foreground font-semibold uppercase tracking-wider block">Đơn tối thiểu</span>
                            <span className="text-xs font-bold text-foreground font-mono mt-0.5">
                              {rule.minBillValue > 0 ? `${rule.minBillValue.toLocaleString()}₫` : 'Vô điều kiện'}
                            </span>
                          </div>

                          <div className="p-2 bg-muted/30 rounded-xl border border-border/10 flex flex-col justify-between">
                            <span className="text-[9px] text-muted-foreground font-semibold uppercase tracking-wider block">Thời hạn mã</span>
                            <span className="text-xs font-bold text-foreground font-mono mt-0.5 flex items-center gap-1">
                              <Calendar className="w-2.5 h-2.5 text-muted-foreground" />
                              {rule.expiryDays ? `${rule.expiryDays} ngày` : '30 ngày'}
                            </span>
                          </div>
                        </div>
          ...
          <div className="xl:col-span-4 space-y-4">
            ...
            <Card className="border border-border bg-card rounded-[1.5rem] shadow-sm text-left">
              <div className="p-5 border-b border-border">
                ...
                <p className="text-[11px] text-muted-foreground mt-0.5">
                  Tạo nhanh các phần quà tiêu biểu để quản lý chiến dịch của bạn tiện lợi hơn.
                </p>
              </div>
              <CardContent className="p-5 space-y-4">
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Hệ thống đề xuất các mẫu chương trình tích điểm đổi quà mang lại tính trung thành cao nhất dựa trên chỉ số chiết khấu tối ưu.
                </p>
                <button
                  type="button"
                  onClick={handleBootstrap}
            ...
            <Card className="border border-indigo-100 bg-indigo-50/10 dark:border-indigo-950 dark:bg-indigo-950/10 rounded-[1.5rem] p-5 text-left space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                Để kiểm thử tính hợp lý nhanh trước khi đưa vào áp dụng thực tế với khách hàng.
              </p>
              ...
              <div className="space-y-1.5 max-h-[160px] overflow-y-auto pr-1">
                {rules.map(rule => {
                  ...
                  return (
                    <div key={rule.id} className="flex items-center justify-between p-2 bg-background/50 rounded-lg text-xs">
                      <span className="font-medium truncate max-w-[120px] text-foreground">{rule.name}</span>
                      {eligible ? (
                        <button
                          onClick={() => handleSimulateClaim(rule)}
                          className="px-2 py-0.5 bg-emerald-500 text-white text-[10px] rounded hover:bg-emerald-600 font-bold"
                        >
                          Thử Đổi
                        </button>
                      ) : (
                        <span className="text-[10px] text-amber-500 font-bold">-{rule.pointsRequired - Number(simPoints)} pts</span>
                      )}
                    </div>
                  );
                })}
          ...
          <div className="xl:col-span-5 space-y-4">
            <Card className="border border-border bg-card rounded-3xl overflow-hidden text-left">
              <div className="p-5 border-b border-border bg-muted/10">
                ...
                <p className="text-[11px] text-muted-foreground mt-0.5">
                   Tìm kiếm khách hàng theo Tên, Số điện thoại hoặc Email để tra cứu ví điểm.
                </p>
              </div>
                      ...
                      return (
                        ...
                        >
                          <div className="space-y-0.5 text-left max-w-[70%]">
                            <span className="font-bold text-xs block text-foreground truncate">{cust.name}</span>
                            <span className="text-[10px] text-muted-foreground font-mono block">SĐT: {cust.phone || 'Chưa cung cấp'}</span>
                          </div>

                          <div className="text-right">
                            <span className="text-xs bg-primary/10 text-primary px-2.5 py-1 rounded-xl font-mono font-black">
            ...
            <Card className="border border-border bg-card rounded-3xl overflow-hidden text-left">
              <div className="p-4 border-b border-border bg-muted/10 flex items-center justify-between">
                ...
                <span className="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full font-bold">LIVE-SYNCED</span>
              </div>
              <CardContent className="p-4">
                {loadingLogs ? (
                  <div className="py-4 text-center text-xs text-muted-foreground">Đang tải lịch sử đổi...</div>
                ) : redeemLogs.length === 0 ? (
                  <p className="text-[11px] text-muted-foreground italic text-center py-4">Chưa có giao dịch đổi quà nào hôm nay.</p>
                ) : (
                  <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1">
                    {redeemLogs.map((log) => (
                      <div key={log.id} className="p-2.5 bg-muted/30 border border-border/20 rounded-xl space-y-1">
                        <div className="flex justify-between items-start">
                          <span className="font-bold text-[11px] text-foreground block truncate max-w-[150px]">{log.ruleName}</span>
                          <span className="text-[10px] text-rose-600 font-extrabold font-mono">-{log.pointsRedeemed} pts</span>
                        </div>
                        <div className="flex justify-between items-center text-[10px] text-muted-foreground">
                          <span>Khách: <strong className="text-foreground">{log.customerName}</strong></span>
                          <span>
                            {log.createdAt?.toDate ? log.createdAt.toDate().toLocaleTimeString("vi-VN", { hour: "numeric", minute: "numeric" }) : "Vừa xong"}
                          </span>
                        </div>
                      </div>
                    ))}
          ...
          <div className="xl:col-span-7 space-y-4">
            {!selectedCustomer ? (
              <div className="py-32 text-center border-2 border-dashed border-border rounded-3xl space-y-4 bg-muted/20">
                ...
                <div className="space-y-1">
                  <p className="text-sm font-bold text-muted-foreground">Bếp Phục Vụ Đổi Quà SEVA</p>
                  <p className="text-xs text-muted-foreground max-w-sm mx-auto p-2">
                    Vui lòng click chọn 1 khách hàng từ danh bạ bên trái để mở giao diện kiểm duyệt, tùy chỉnh điểm số và thực hiện trừ điểm tự động khi quy nạp quà thưởng.
                  </p>
                </div>
              </div>
            ) : (
              ...
              >
                  ...
                  <CardContent className="p-6 relative text-left">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-0.5">
                          <h4 className="font-bold text-base leading-none text-white">{selectedCustomer.name}</h4>
                          <span className="text-xs text-white/70 block">SĐT: {selectedCustomer.phone || "Không có"}</span>
                          <span className="text-[10px] bg-white/20 text-white px-2 py-0.5 rounded font-black tracking-wide uppercase inline-block">
                            {selectedCustomer.points && selectedCustomer.points >= 150000 ? 'Atelier Tier' : selectedCustomer.points && selectedCustomer.points >= 50000 ? 'Icon Tier' : 'Essential Tier'}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="text-right flex flex-col items-end shrink-0 sm:border-l sm:border-white/10 sm:pl-6">
                        <span className="text-[10px] text-white/70 font-bold uppercase tracking-widest block">Số Điểm Hiện Có</span>
                        <div className="flex items-baseline gap-1 mt-0.5">
                          ...
                          <span className="text-xs text-white/80 font-mono font-bold">PTS</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-6 pt-4 border-t border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white/5 -mx-6 -mb-6 p-4">
                      <div className="flex items-center gap-1.5 text-xs text-white/80">
                        ...
                        <span>Hỗ trợ Demo: Nhanh chóng nạp/khấu trừ để test các ngưỡng đổi thưởng.</span>
                      </div>
                      <div className="flex gap-2 justify-end">
                        <button
                          onClick={() => handleAdjustCustomerPoints(selectedCustomer, -100)}
                          className="px-2.5 py-1 bg-white/10 hover:bg-white/15 border border-white/5 rounded-lg text-[10px] font-bold text-white transition-all cursor-pointer flex items-center gap-1"
                        ...
                        <button
                          onClick={() => handleAdjustCustomerPoints(selectedCustomer, 500)}
                          className="px-2.5 py-1 bg-white hover:bg-white/90 rounded-lg text-[10px] font-bold text-primary transition-all cursor-pointer flex items-center gap-1 shadow-sm"
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center px-1">
                    <h5 className="text-[11px] font-extrabold text-[#2f6cf5] uppercase tracking-widest text-left">
                      Khả năng đổi điểm nhận quà của khách hàng
                    </h5>
                    <span className="text-[10px] text-muted-foreground font-semibold">Tích có sẵn: {(selectedCustomer.points || 0).toLocaleString()} pts</span>
                  </div>

                  {rules.length === 0 ? (
                    <div className="py-12 bg-card rounded-3xl text-center text-xs text-muted-foreground border">
                      Chưa cấu hình bất kỳ ưu đãi quà tặng nào trong Thư viện Ưu đãi.
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 gap-3">
                      {rules.map((rule: any) => {
                        const currentPoints = Number(selectedCustomer.points || 0);
                        const isEligible = currentPoints >= rule.pointsRequired && rule.isEnabled !== false;
                        const missingPoints = rule.pointsRequired - currentPoints;
                        const progressPercentage = Math.min(100, Math.floor((currentPoints / rule.pointsRequired) * 100));
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-start gap-3.5">
                              <div className={`p-2.5 rounded-2xl ${isEligible ? 'bg-emerald-500/10 text-emerald-600' : 'bg-muted text-muted-foreground'} shrink-0 mt-0.5`}>
                                <TypeIcon className="w-5 h-5" />
                              </div>
                              <div className="space-y-1">
                                <span className="font-bold text-xs text-foreground block">{rule.name}</span>
                                <span className="text-[10px] text-muted-foreground block max-w-sm">
                                  {rule.description || 'Không có hướng dẫn phụ.'}
                                </span>
                                <span className="text-[10px] font-semibold text-primary font-mono block">
                                  Quy đổi: <strong className="font-bold">{rule.pointsRequired} pts</strong>
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="shrink-0 self-end md:self-center">
                              {rule.isEnabled === false ? (
                                <span className="px-2 py-1 text-[9px] bg-slate-100 text-slate-500 rounded-md font-extrabold uppercase shrink-0 tracking-wider">
                                  Tạm dừng
                                </span>
                              ) : isEligible ? (
                                <button
                                  type="button"
                                  onClick={() => handleExecuteRealRedeem(selectedCustomer, rule)}
                                ...
                                <div className="space-y-1.5 text-right w-44">
                                  ...
                                  <div className="flex justify-between items-center text-[9px] font-mono text-muted-foreground">
                                    ...
                                    <span className="text-amber-600 font-bold">Thiếu {missingPoints} pts</span>
                                  </div>
                                </div>
                              )}
                            </div>
              ...
              <form onSubmit={handleSaveRule} className="flex flex-col h-full">
                ...
                <div className="p-6 border-b border-border bg-muted/10 flex items-center justify-between animate-in fade-in">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="font-bold text-base text-foreground">Thiết lập Quà Tặng / Ưu Đãi</h4>
                      <p className="text-[11px] text-muted-foreground">Khai báo phần thưởng trong thư viện trước khi khách tiến hành đổi.</p>
                    </div>
                  </div>
                  <button 
                    type="button" 
                ...
                <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-muted-foreground uppercase tracking-tight">Tên quà tặng / Ưu đãi</label>
                    <input 
                      type="text"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-muted-foreground uppercase tracking-tight">Mô tả quà tặng & Hướng dẫn sử dụng</label>
                    <textarea 
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-muted-foreground uppercase tracking-tight">Nhóm Quà Ưu Đãi</label>
                      <select
                        value={editingRule.rewardType}
                        onChange={e => setEditingRule({ ...editingRule, rewardType: e.target.value as any })}
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-muted-foreground uppercase tracking-tight">Giá trị (VNĐ hoặc %)</label>
                      <input 
                        type="number"
                        disabled={editingRule.rewardType === 'item' || editingRule.rewardType === 'service'}
                        value={editingRule.rewardValue}
                        onChange={e => setEditingRule({ ...editingRule, rewardValue: Number(e.target.value) })}
                        className="w-full bg-background border border-border rounded-xl px-3 py-2 text-xs font-mono outline-none focus:border-primary/50 disabled:bg-muted/50 disabled:cursor-not-allowed text-foreground"
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-1">
                    <div className="space-y-1">
                      ...
                      <input 
                        ...
                        onChange={e => setEditingRule({ ...editingRule, pointsRequired: Number(e.target.value) })}
                        className="w-full bg-background border border-border rounded-xl px-3 py-2 text-xs font-mono outline-none focus:border-primary/50 text-foreground font-bold"
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-muted-foreground uppercase tracking-tight">Hạn dùng sau khi nhận (Ngày)</label>
                      <input 
                        type="number"
                        required
                        value={editingRule.expiryDays || 30}
                        onChange={e => setEditingRule({ ...editingRule, expiryDays: Number(e.target.value) })}
                        className="w-full bg-background border border-border rounded-xl px-3 py-2 text-xs font-mono outline-none focus:border-primary/50 text-foreground"
                  ...
                  <div className="space-y-1 pt-1">
                    <label className="text-xs font-bold text-muted-foreground uppercase tracking-tight">Giá trị đơn hàng tối thiểu áp dụng (VNĐ)</label>
                    <input 
                      type="number"
                      value={editingRule.minBillValue || 0}
                      onChange={e => setEditingRule({ ...editingRule, minBillValue: Number(e.target.value) })}
                      placeholder="Để 0 nếu không hạn chế"
                      className="w-full bg-background border border-border rounded-xl px-3.5 py-2 text-xs font-mono outline-none focus:border-primary/50 text-foreground"
                    />
                    <p className="text-[10px] text-muted-foreground italic">Giúp kiểm soát tối ưu ngân sách chiết khấu.</p>
                  </div>

                  <div className="p-4 bg-muted/40 rounded-2xl border border-border/20 flex items-center justify-between">
                    <div>
                      <p className="font-bold text-xs text-foreground">Trạng thái phát hành</p>
                      <p className="text-[10px] text-muted-foreground">Kích hoạt để cho phép đổi điểm lấy ưu đãi này</p>
                    </div>
                    <button 
                      type="button"
                      onClick={() => setEditingRule({ ...editingRule, isEnabled: editingRule.isEnabled !== false ? false : true })}
                    ...
                    >
                      <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-transform shadow-sm ${editingRule.isEnabled !== false ? 'left-6' : 'left-1'}`} />
                    </button>
                  </div>
