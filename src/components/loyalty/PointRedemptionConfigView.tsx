...
export function PointRedemptionConfigView() {
 ...
 <p className="text-xs text-muted-foreground ">Đã khấu trừ: -{rule.pointsRequired} pts. Điểm ảo còn lại: {nextPoints} pts</p>
 </div>,
 { duration: 4000 }
 );
 };
 ...
 <p className="text-xs text-muted-foreground">Tạo các gói kích cầu mua sắm bằng điểm của khách hàng</p>
 </div>
 ...
 <span className="bg-rose-500/10 text-rose-600 font-extrabold text-xs tracking-wider px-1.5 py-0.2 rounded uppercase">Vô hiệu hóa</span>
 )}
 </div>
 ...
 <span className="text-xs text-muted-foreground font-semibold uppercase tracking-wider block">Yêu cầu đổi</span>
 ...
 <span className="text-xs text-muted-foreground font-semibold uppercase tracking-wider block">Giá trị gói quà</span>
 ...
 <div className="p-2 bg-muted/30 rounded-xl border border-border/10 flex flex-col justify-between">
 <span className="text-xs text-muted-foreground font-semibold uppercase tracking-wider block">Đơn tối thiểu</span>
 <span className="text-xs font-bold text-foreground mt-0.5">
 ...
 <div className="p-2 bg-muted/30 rounded-xl border border-border/10 flex flex-col justify-between">
 <span className="text-xs text-muted-foreground font-semibold uppercase tracking-wider block">Thời hạn mã</span>
 ...
 <span className="text-xs text-amber-500 font-bold">-{rule.pointsRequired - Number(simPoints)} pts</span>
 )}
 </div>
 );
 })}
 ...
 <span className="text-xs text-muted-foreground block">SĐT: {cust.phone || 'Chưa cung cấp'}</span>
 </div>
 ...
 <span className="text-xs bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full font-bold">LIVE-SYNCED</span>
 </div>
 <CardContent className="p-4">
 {loadingLogs ? (
 <div className="py-4 text-center text-xs text-muted-foreground">Đang tải lịch sử đổi...</div>
 ) : redeemLogs.length === 0 ? (
 <p className="text-xs text-muted-foreground italic text-center py-4">Chưa có giao dịch đổi quà nào hôm nay.</p>
 ) : (
 <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1">
 {redeemLogs.map((log) => (
 <div key={log.id} className="p-2.5 bg-muted/30 border border-border/20 rounded-xl space-y-1">
 <div className="flex justify-between items-start">
 <span className="font-bold text-xs text-foreground block truncate max-w-[150px]">{log.ruleName}</span>
 <span className="text-xs text-rose-600 font-extrabold ">-{log.pointsRedeemed} pts</span>
 </div>
 ...
 <span className="text-xs text-white/70 font-bold uppercase tracking-widest block">Số Điểm Hiện Có</span>
 ...
 <span className="text-xs text-white/80 font-bold">PTS</span>
 </div>
 </div>
 </div>
 ...
 <span className="text-xs text-muted-foreground font-semibold">Tích có sẵn: {(selectedCustomer.points || 0).toLocaleString()} pts</span>
 </div>
 ...
 <p className="text-xs text-muted-foreground">Khai báo phần thưởng trong thư viện trước khi khách tiến hành đổi.</p>
 </div>
 </div>
 <button 
 type="button" 
 ...
 />
 <p className="text-xs text-muted-foreground italic">Giúp kiểm soát tối ưu ngân sách chiết khấu.</p>
 </div>
 ...
 <p className="text-xs text-muted-foreground">Kích hoạt để cho phép đổi điểm lấy ưu đãi này</p>
 </div>
 <button 
 type="button"
