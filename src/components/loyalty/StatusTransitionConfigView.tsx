      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
        ...
        <div className="xl:col-span-7 space-y-4">
          <div className="flex items-center justify-between px-1">
             <h5 className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">Danh sách quy tắc kích hoạt ({rules.length})</h5>
             <span className="text-[10px] bg-emerald-500/10 text-emerald-600 px-2 py-0.5 rounded-md font-bold">Thời gian thực (Firestore)</span>
          </div>
                  ...
                  <Card key={rule.id} className={`border-none ${rule.enabled ? 'bg-card' : 'bg-muted/10 opacity-70'} hover:shadow-lg transition-all duration-300 rounded-3xl overflow-hidden border border-border/40`}>
                    <CardContent className="p-6 space-y-4">
                      <div className="flex items-start justify-between gap-4">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {!rule.enabled && <span className="text-[9px] bg-red-500/10 text-red-600 px-1.5 py-0.2 rounded font-black tracking-wider uppercase">TẮT</span>}
                          </div>
                      ...
                      <div className="p-3 bg-muted/30 rounded-2xl flex items-center justify-between border border-border/20 text-xs">
                        <div className="flex items-center gap-2 font-mono">
                          ...
                          {rule.fromStatus === "ALL" ? (
                            ...
                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${sourceStatusObj?.color.bg || ''} ${sourceStatusObj?.color.text || ''} ${sourceStatusObj?.color.border || ''}`}>
                              {rule.fromStatus}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-2 font-mono">
                          ...
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${destStatusObj?.color.bg || ''} ${destStatusObj?.color.text || ''} ${destStatusObj?.color.border || ''}`}>
                            {rule.toStatus}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1.5 text-left">
                        <p className="text-[10px] text-muted-foreground font-bold tracking-wider uppercase mb-1">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {rule.conditions.map((cond, index) => {
                            ...
                            return (
                              <span key={index} className="inline-flex items-center gap-1.5 text-[11px] bg-muted px-3 py-1.5 rounded-xl border border-border/40 font-semibold">
                                <span className="text-muted-foreground">{opt?.label}</span>
                                <span className="text-primary font-mono">{operatorText}</span>
                                <span className="font-mono text-foreground font-bold">{niceValue}</span>
                              </span>
                            );
                          })}
                        </div>
            ...
            <CardContent className="p-6 space-y-5 text-left">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] text-muted-foreground font-bold tracking-tight uppercase">Trạng thái hiện tại</label>
                  <select 
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] text-muted-foreground font-bold tracking-tight uppercase">Hạng hội viên hiện có</label>
                  <select 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] text-muted-foreground font-bold tracking-tight uppercase">Tổng chi tiêu (₫)</label>
                  <input 
                    type="number" 
                    value={simSpend}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] text-muted-foreground font-bold tracking-tight uppercase">Điểm tích lũy (pts)</label>
                  <input 
                    type="number" 
                    value={simPoints}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] text-muted-foreground font-bold tracking-tight uppercase">Số ngày không hoạt động</label>
                  <input 
                    type="number" 
                    value={simInactive}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] text-muted-foreground font-bold tracking-tight uppercase">Tắt giảm giao dịch %</label>
                  <input 
                    type="number" 
                    value={simDropRate}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] text-muted-foreground font-bold tracking-tight uppercase">Số khiếu nại/xấu</label>
                  <input 
                    type="number" 
                    value={simBadReviews}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] text-muted-foreground font-bold tracking-tight uppercase">Cảnh báo nghi ngờ bảo mật</label>
                  <input 
                    type="number" 
              ...
              {simResults.length > 0 && (
                <div className="mt-5 space-y-4 pt-4 border-t border-border">
                  <h6 className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">Kế quả đối chiếu điều kiện</h6>
                      ...
                      <div key={res.ruleId} className={`p-4 rounded-2xl border ${res.isMatched ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-950 dark:text-emerald-100' : 'bg-muted/40 border-border/40 opacity-75'} text-xs`}>
                        <div className="flex items-start justify-between gap-2.5">
                          <div className="space-y-0.5">
                            <span className="font-bold block text-[13px]">{res.ruleName}</span>
                            <span className="text-[10px] text-muted-foreground block font-mono">ID: {res.ruleId}</span>
                          </div>
                          <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase shrink-0 ${res.isMatched ? 'bg-emerald-500 text-white' : 'bg-muted text-muted-foreground'}`}>
                            {res.isMatched ? 'KHỚP ĐIỀU KIỆN' : 'KHÔNG KHỚP'}
                          </span>
                        </div>

                        {/* Breakdown conditions detail */}
                        <div className="mt-3.5 pl-2.5 border-l-2 border-border space-y-1.5 text-muted-foreground font-mono text-[10px] text-left">
                        ...
                        {res.isMatched && (
                          <div className="mt-4 pt-3 border-t border-emerald-500/20 space-y-2">
                            ...
                            {res.appliedAutomations.length > 0 && (
                              <div className="space-y-1">
                                <p className="text-[9px] text-muted-foreground uppercase font-semibold">Tự động hóa hành vi kích hoạt:</p>
                                <div className="flex flex-wrap gap-1">
                                  {res.appliedAutomations.map((auto, i) => (
                                    <span key={i} className="bg-emerald-500/20 text-emerald-800 dark:text-emerald-300 px-1.5 py-0.5 rounded text-[9px] font-semibold">{auto}</span>
                                  ))}
                                </div>
                              </div>
                            )}
              ...
              {executionReport && (
                ...
                >
                  <div className="flex items-center justify-between">
                    <h6 className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">Báo cáo cập nhật trực tiếp</h6>
                    <span className="text-[10px] bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-0.5 px-2 rounded-full">
                      Hoàn tất
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3.5 text-center">
                    <div className="bg-muted/40 p-3 rounded-2xl border border-border/10">
                      <span className="text-[10px] text-muted-foreground uppercase block font-medium">Tổng số khách hàng quét</span>
                      <span className="text-lg font-mono font-bold text-foreground">{executionReport.totalCustomers}</span>
                    </div>
                    <div className="bg-emerald-500/10 p-3 rounded-2xl border border-emerald-500/15 text-emerald-800 dark:text-emerald-400">
                      <span className="text-[10px] text-muted-foreground uppercase block font-medium">Khách hàng được cập nhật</span>
                      <span className="text-lg font-mono font-bold">{executionReport.changedCount}</span>
                    </div>
                  </div>
                  ...
                  {executionReport.details.length > 0 ? (
                    <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                      {executionReport.details.map((log, i) => (
                        <div key={i} className="p-3 bg-muted/30 rounded-xl border border-border/30 text-[11px] space-y-1">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-foreground">{log.customerName}</span>
                            <span className="text-[9px] text-[#2f6cf5] font-semibold">{log.ruleName}</span>
                          </div>
                          <div className="flex items-center gap-1.5 text-xs text-muted-foreground font-mono">
                            <span className="bg-amber-500/10 text-amber-700 dark:text-amber-400 px-1.5 py-0.2 rounded text-[9px] font-bold">{log.fromStatus}</span>
                            <ArrowRight className="w-3 h-3 text-muted-foreground" />
                            <span className="bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 px-1.5 py-0.2 rounded text-[9px] font-bold">{log.toStatus}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="p-6 border-b border-border bg-muted/10 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground">Xây dựng luật tự động đổi nhãn phân loại khách hàng với nhiều thuộc tính.</p>
                    </div>
                  </div>
                  <button 
                    type="button" 
                            ...
                            {cond.metric === 'current_tier' ? (
                              ...
                              <div className="relative w-full sm:w-44">
                                <input
                                  ...
                                  value={cond.value}
                                  onChange={e => handleUpdateCondition(index, "value", e.target.value)}
                                  className="w-full bg-background border border-border rounded-xl pl-3 pr-8 py-2 text-xs font-mono outline-none focus:border-primary/50"
                                  placeholder="0"
                                />
                                {currentMetricType?.unit && (
                                  <span className="absolute right-3.5 top-2.5 text-[10px] text-muted-foreground font-mono font-bold">
                                    {currentMetricType.unit}
                                  </span>
                                )}
                              </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                      <label className="flex items-center gap-3 p-3 bg-muted/15 border border-border/20 rounded-2xl cursor-pointer hover:bg-muted/30 transition-colors">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-muted-foreground">Tự động nhắn tin cá nhân hóa thương hiệu</span>
                        </div>
                      </label>
                      ...
                      <label className="flex items-center gap-3 p-3 bg-muted/15 border border-border/20 rounded-2xl cursor-pointer hover:bg-muted/30 transition-colors">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-muted-foreground">Tặng ưu đãi giảm 20% tự động</span>
                        </div>
                      </label>
                      ...
                      <label className="flex items-center gap-3 p-3 bg-muted/15 border border-border/20 rounded-2xl cursor-pointer hover:bg-muted/30 transition-colors">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-muted-foreground">Gửi thông báo đẩy lên dashboard nhân viên CSKH</span>
                        </div>
                      </label>
                      ...
                      <label className="flex items-center gap-3 p-3 bg-muted/15 border border-border/20 rounded-2xl cursor-pointer hover:bg-muted/30 transition-colors">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-muted-foreground">Tăng tốc độ nhận ưu đãi dành cho hội viên</span>
                        </div>
                      </label>
                      ...
                      <label className="flex items-center gap-3 p-3 bg-muted/15 border border-border/20 rounded-2xl cursor-pointer hover:bg-muted/30 transition-colors sm:col-span-2">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-muted-foreground">Chặn đổi điểm, login sảnh khách hàng hoặc giao dịch thanh toán để đề phòng gian lận</span>
                        </div>
                      </label>
                    </div>
                  </div>
