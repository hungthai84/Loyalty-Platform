...
export function RoleManager() {
 ...
 <span className="text-xs text-muted-foreground leading-none block">{u.email}</span>
 </div>
 </div>
 </div>

 <div className="flex items-center gap-2 pt-1">
 <label className="text-xs font-black uppercase text-muted-foreground shrink-0">Chọn vai trò:</label>
 <select
 value={selectedRole}
 ...
 <p className="text-xs text-muted-foreground">Tài khoản Gmail hợp lệ được cấp quyền gia nhập hệ thống CRM.</p>
 </div>
 ...
 <span className="text-xs text-muted-foreground block pl-7">{u.email}</span>
 </div>

 <div className="flex items-center gap-1">
 {u.email?.toLowerCase() === "hungthai84@gmail.com" ? (
 <span className="p-1 px-2.5 border rounded-lg bg-emerald-500/10 text-emerald-600 border-emerald-500/25 text-xs font-black uppercase">Gốc</span>
 ) : (
 <>
 <select
 ...
 <p className="text-xs text-muted-foreground">Tài khoản nằm trong diện từ chối hoặc đã thu hồi quyền.</p>
 </div>
 ...
 <span className="text-xs text-muted-foreground/60 block">{u.email}</span>
 </div>
 <button
