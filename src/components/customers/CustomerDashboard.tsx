...
export function CustomerDashboard({ customer, userId, companies, attributes, onBack }: CustomerDashboardProps) {
 ...
 <span className={`px-2.5 py-0.5 text-xs font-bold uppercase rounded-full border ${tier.color} ${tier.bg}`}>
 🏆 {tier.name}
 </span>
 </div>
 </div>
 ...
 <h4 className="text-xs font-bold uppercase text-[#2f6cf5] tracking-wider mb-2">Sửa thông tin cơ bản</h4>
 
 <div className="space-y-2 bg-background/50 p-3 rounded-xl border border-dashed border-border/80">
 <span className="text-xs text-muted-foreground block font-bold uppercase tracking-wider mb-1">CẬP NHẬT ẢNH ĐẠI DIỆN</span>
 ...
 <span className="text-xs text-muted-foreground">Vui lòng tải tệp ảnh từ máy</span>
 </div>
 </div>
 <div className="space-y-1">
 <span className="text-xs text-muted-foreground block font-bold">SỐ ĐIỆN THOẠI</span>
 <input 
 ...
 <div className="space-y-1">
 <span className="text-xs text-muted-foreground block font-bold">EMAIL KHÁCH HÀNG</span>
 <input 
 className="w-full p-2 text-xs bg-background border rounded-lg focus:ring-1 focus:ring-primary/20 outline-none"
 value={email}
 ...
 <p className="text-xs text-muted-foreground truncate">{company.address || 'Không địa chỉ'}</p>
 </div>
 </div>
 )}
 </div>
 ...
 <div>
 <span className="text-xs font-bold text-[#2f6cf5] uppercase tracking-widest block">QUỸ LOYALTY POINTS</span>
 ...
 <span className="text-xs font-extrabold text-muted-foreground uppercase tracking-wider block mb-2">Cập nhật nhanh điểm số (Simulate)</span>
 ...
 >
 <div>
 <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest block">LỘ TRÌNH ĐẶC QUYỀN VIP</span>
 <div className="mt-3 flex items-center justify-between">
 <span className="text-sm font-bold text-foreground capitalize">{tier.name.split(" ")[0]} Member</span>
 <span className="text-xs font-bold text-muted-foreground">{tier.nextTarget}</span>
 </div>
 ...
 <div className="min-w-0">
 <p className="text-xs font-bold text-foreground">Sử dụng điểm đổi quà cao cấp</p>
 <p className="text-xs text-muted-foreground">Phòng chờ VIP, Sự kiện kín tại showroom</p>
 </div>
 </div>
 </motion.div>
 </div>
 ...
 <p className="text-xs text-muted-foreground mt-0.5">Xác thực tài khoản và liên kết dữ liệu đa điểm của khách hàng.</p>
 </div>
 <button 
 onClick={() => {
 if (isEditingSocial) {
 ...
 <div className="space-y-1">
 <span className="text-xs text-muted-foreground font-bold uppercase">FB / Messenger Link</span>
 ...
 <div className="space-y-1">
 <span className="text-xs text-muted-foreground font-bold uppercase">Zalo Sđt / Profile URL</span>
 ...
 <div className="space-y-1">
 <span className="text-xs text-muted-foreground font-bold uppercase">LinkedIn URL</span>
 ...
 <div className="space-y-1">
 <span className="text-xs text-muted-foreground font-bold uppercase">Instagram Handler</span>
 ...
 <span className="text-xs text-muted-foreground font-bold uppercase">TikTok Profile</span>
 ...
 <span className="text-xs font-bold block truncate max-w-full">Facebook</span>
 {fb ? (
 <a href={fb.startsWith('http') ? fb : `https://${fb}`} target="_blank" rel="noreferrer" className="mt-2 text-xs font-extrabold flex items-center gap-0.5 hover:underline uppercase text-blue-700">
 Liên kết <ExternalLink className="w-2 h-2" />
 </a>
 ) : (
 <span className="mt-2 text-xs font-extrabold uppercase opacity-40">Trống</span>
 )}
 </div>
 ...
 <span className="text-xs font-bold block truncate max-w-full">Zalo Chat</span>
 {zl ? (
 <a href={zl.startsWith('http') ? zl : `https://zalo.me/${zl}`} target="_blank" rel="noreferrer" className="mt-2 text-xs font-extrabold flex items-center gap-0.5 hover:underline uppercase text-sky-700">
 ...
 <span className="text-xs font-bold block truncate max-w-full">LinkedIn</span>
 {li ? (
 <a href={li.startsWith('http') ? li : `https://${li}`} target="_blank" rel="noreferrer" className="mt-2 text-xs font-extrabold flex items-center gap-0.5 hover:underline uppercase text-blue-800">
 ...
 <span className="text-xs font-bold block truncate max-w-full">Instagram</span>
 {ig ? (
 <a href={ig.startsWith('http') ? ig : `https://${ig}`} target="_blank" rel="noreferrer" className="mt-2 text-xs font-extrabold flex items-center gap-0.5 hover:underline uppercase text-pink-700">
 ...
 <div className="w-6 h-6 rounded-full bg-slate-800 dark:bg-slate-200 text-white dark:text-black font-extrabold text-xs flex items-center justify-center mb-2 ">♬</div>
 <span className="text-xs font-bold block truncate max-w-full">TikTok</span>
 {tt ? (
 <a href={tt.startsWith('http') ? tt : `https://${tt}`} target="_blank" rel="noreferrer" className="mt-2 text-xs font-extrabold flex items-center gap-0.5 hover:underline uppercase text-slate-900">
 ...
 <span className="text-xs font-bold text-muted-foreground uppercase px-2">Kịch bản:</span>
 <button
 ...
 <span className="text-xs text-muted-foreground block font-bold uppercase tracking-wider">ĐÃ CHI TIÊU</span>
 ...
 <div className="bg-background/45 p-3 rounded-2xl border border-border/40">
 <span className="text-xs text-muted-foreground block font-bold uppercase tracking-wider">CLV LŨY KẾ DỰ DỰ KIẾN</span>
 ...
 <span className="text-xs text-muted-foreground block font-bold uppercase tracking-wider">TĂNG TRƯỞNG DỰ KIẾN</span>
 <span className="text-sm font-black text-emerald-500 block mt-1 flex items-center gap-1">
 +{formatVND(clvStats.growthAmount)}
 <span className="text-xs font-bold text-emerald-500">({(clvStats.growthRate * 100).toFixed(1)}%)</span>
 </span>
 <span className="text-xs text-muted-foreground mt-0.5 block flex items-center gap-1">
 ...
 <span className="font-bold text-foreground">{formatVND(data["Đã Chi Tiêu"])}</span>
 </div>
 )}
 ...
 <span className="font-bold text-[#2f6cf5]">{formatVND(data["Dự Báo CLV"])}</span>
 </div>
 )}
 {!isHistorical && (
 ...
 <p className="text-xs text-muted-foreground mt-0.5">Biên niên sử đồng bộ thời gian thực hiển thị giao dịch, khiếu nại & biến động trạng thái.</p>
 </div>
 ...
 <span className={`inline-block text-xs px-1.5 py-0.5 rounded border font-semibold ${item.badgeStyle}`}>
 {item.badgeText}
 </span>
 </div>
 ...
 <p className="text-xs text-muted-foreground mt-0.5">Dữ liệu đơn hàng đồng bộ trực tiếp từ hệ thống ERP/CRM.</p>
 </div>
 <button 
 onClick={async () => {
 try {
 ...
 <span className="text-xs text-muted-foreground mt-1 max-w-xs">Hệ thống chưa ghi nhận phát sinh đơn hàng nào hoặc API chưa đồng bộ dữ liệu.</span>
 </div>
 ) : (
 ...
 <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${order.statusClasses}`}>
 {order.status}
 </span>
 </div>
 ...
 <p className="text-xs text-muted-foreground mt-0.5">Lịch sử khiếu nại, bảo hành và yêu cầu hỗ trợ từ khách hàng.</p>
 </div>
 <button 
 onClick={async () => {
 try {
 ...
 <span className="text-xs text-muted-foreground mt-1 max-w-xs">Khách hàng chưa có yêu cầu hỗ trợ hoặc khiếu nại nào được ghi nhận.</span>
 </div>
 ) : (
 ...
 <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${ticket.status === 'Đang xử lý' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' : 'bg-slate-500/10 text-slate-500 border-slate-500/20'}`}>
 {ticket.status}
 </span>
 </div>
 <span className={`text-xs font-bold px-2 py-0.5 rounded border ${ticket.severity === 'Cao' ? 'bg-rose-500/10 text-rose-500 border-rose-500/20' : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'}`}>
 {ticket.severity}
 </span>
 </div>
 <div className="flex items-center justify-between text-xs text-muted-foreground">
