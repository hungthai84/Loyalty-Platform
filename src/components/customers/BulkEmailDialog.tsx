      ...
      <DialogContent className="sm:max-w-[600px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-4">
          ...
          <div className={`space-y-2 ${!showPreview && "max-md:hidden"}`}>
            ...
            <div className="rounded-[10px] border border-border/50 bg-muted/30 p-4 min-h-[220px] h-[260px] overflow-y-auto">
        ...
        <DialogFooter className="flex flex-col sm:flex-row gap-3 mt-2">
          <Button variant="ghost" onClick={onClose} className="rounded-[10px] font-bold h-10">Hủy bỏ</Button>
          <Button 
            onClick={handleSend} 
            disabled={isSending}
            className="rounded-[10px] font-bold px-6 bg-primary shadow-lg shadow-primary/20 h-10"
          >
            {isSending ? (
              <>Đang gửi...</>
            ) : (
